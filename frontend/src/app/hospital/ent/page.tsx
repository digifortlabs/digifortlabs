...
export default function ENTDashboard() {
    ...
    useEffect(() => {
        ...
        setUserRole(role);
        const saved = localStorage.getItem('ent_hospital_id');
        if (saved) setSelectedHospitalId(Number(saved));

        const handleHospitalChanged = (e: any) => {
            if (e.detail?.storageKey === 'ent_hospital_id') {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const [patientsData, statsData, surgeriesData] = await Promise.all([
                    apiFetch(`ent/patients${suffix}`),
                    apiFetch(`ent/stats${suffix}`),
                    apiFetch(`ent/surgeries${suffix}`)
                ]);
    ...
    if (['website_admin', 'superadmin', 'superadmin_staff'].includes(userRole) && !selectedHospitalId) {
        return <HospitalSelectionPrompt requiredModule="ent" storageKey="ent_hospital_id" onSelect={setSelectedHospitalId} />;
    }
        ...
        <div className="p-6 space-y-6 max-w-7xl mx-auto pb-24">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                    {['website_admin', 'superadmin', 'superadmin_staff'].includes(userRole) && selectedHospitalId && (
                        <Button
                            onClick={() => {
                                ...
                                localStorage.removeItem('ent_hospital_id');
                            }}
                            variant="outline"
                    ...
                    <Button onClick={() => router.push('/hospital/patients')} className="bg-emerald-600 hover:bg-emerald-700 shadow-sm text-white hidden sm:flex">
