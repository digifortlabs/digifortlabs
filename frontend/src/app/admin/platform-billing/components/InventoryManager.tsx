...
export default function InventoryManager() {
    ...
    const fetchInventory = async () => {
        setLoading(true);
        try {
            const hid = localStorage.getItem('globalHospitalId');
            const suffix = hid ? `?hospital_id=${hid}` : '';
            const data = await apiFetch(`/inventory/${suffix}`);
            setItems(data);
        } catch (error) {
            console.error(error);
        } finally {
    ...
    const handleAddItem = async () => {
        try {
            const hid = localStorage.getItem('globalHospitalId');
            const suffix = hid ? `?hospital_id=${hid}` : '';
            await apiFetch(`/inventory/${suffix}`, {
                method: 'POST',
                body: JSON.stringify({
                    ...newItem,
