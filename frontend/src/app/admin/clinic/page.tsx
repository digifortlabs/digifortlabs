...
export default function ClinicOPDDashboard() {
    ...
    useEffect(() => {
        ...
        setUserRole(role);
        const saved = localStorage.getItem('clinic_hospital_id');
        if (saved) setSelectedHospitalId(Number(saved));

        const handleHospitalChanged = (e: any) => {
            if (e.detail?.storageKey === 'clinic_hospital_id') {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                const [patientsData, statsData] = await Promise.all([
                    apiFetch(`clinic/patients${suffix}`),
                    apiFetch(`clinic/stats${suffix}`)
                ]);
    ...
    const fetchGlobalPatients = async (query: string) => {
        ...
        try {
            let suffix = selectedHospitalId ? `&hospital_id=${selectedHospitalId}` : '';
            const data = await apiFetch(`patients?search=${encodeURIComponent(query)}${suffix}`);
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            if (data) {
                ...
                setGlobalSearch('');
                // Refresh list
                const refreshed = await apiFetch(`clinic/patients${suffix}`);
                    ...
                    {['website_admin', 'superadmin', 'superadmin_staff'].includes(userRole) && selectedHospitalId && (
                        <Button
                            onClick={() => {
                                ...
                                localStorage.removeItem('clinic_hospital_id');
                            }}
                            variant="outline"
                                ...
                                {loading ? (
                                    ...
                                    filteredPatients.map(patient => (
                                        <div
                                            ...
                                            onClick={() => router.push(`/admin/clinic/${patient.patient_id}`)}
