...
export default function InvoicePreviewPage() {
    ...
    return (
        <div className="min-h-screen bg-slate-100/50 print:bg-white flex flex-col items-center font-sans antialiased text-slate-900">
            <style jsx global>{`
                @media print {
                    @page {
                        size: A4;
            ...
            {/* Toolbar - Floating & Non-printable */}
            <div className="sticky top-4 z-[100] w-full max-w-[210mm] px-4 print-hidden">
                <div className="bg-white/90 backdrop-blur-xl border border-slate-200 p-4 rounded-3xl shadow-2xl flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-500 font-mono tracking-tighter">{invoice.invoice_number}</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-5 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl text-sm font-bold hover:bg-indigo-100 transition-colors"
                        >
            ...
            <div
                id="invoice-render"
                className="bg-white shadow-2xl p-12 border border-slate-200 print:shadow-none print:border-none print:p-12 print:mx-0 w-[210mm] box-border my-8 print:my-0"
            >
                {/* PAGE 1: INVOICE SUMMARY */}
                <div className="min-h-[297mm] print:min-h-screen flex flex-col print:break-after-page relative">
                    {/* Brand Header */}
                    <div className="flex justify-between items-start mb-8">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-[11px] text-slate-500 leading-relaxed max-w-[200px] ml-auto">
                    ...
                    {/* Tax Invoice Banner */}
                    <div className="bg-slate-900 text-white text-center py-2.5 font-black tracking-[0.3em] mb-0 text-[10px] uppercase border border-slate-900">
                    ...
                    <div className="grid grid-cols-2 border-x border-b border-slate-900">
                        <div className="p-6 border-r border-slate-900">
                            <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Customer Details (Billed To)</h3>
                            <div className="font-black text-slate-900 text-base">{invoice.hospital_name}</div>
                            <div className="text-[11px] text-slate-600 mt-2 space-y-1 leading-relaxed">
                                ...
                                <div className="pt-3 space-y-1">
                                    <p><span className="text-slate-400 font-bold uppercase mr-2 text-[9px]">GSTIN/UIN:</span><span className="font-black text-slate-900">{invoice.hospital_gst || 'URD'}</span></p>
                                    <p><span className="text-slate-400 font-bold uppercase mr-2 text-[9px]">PAN:</span><span className="font-black text-slate-900">{invoice.hospital_pan || '-'}</span></p>
                                </div>
                            </div>
                        </div>
                        <div className="p-6">
                            <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Invoice & Payment Details</h3>
                            <div className="text-[11px] space-y-3">
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-500 font-bold uppercase text-[9px]">Invoice Number:</span>
                                    <span className="font-black text-slate-900 text-sm tracking-tight">{invoice.invoice_number}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-500 font-bold uppercase text-[9px]">Bill Date:</span>
                                ...
                                <div className="flex justify-between items-center pt-3 border-t border-slate-100">
                                    <span className="text-slate-500 font-bold uppercase text-[9px]">Place of Supply:</span>
                                    <span className="font-black text-slate-900 uppercase">GUJARAT (24)</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-500 font-bold uppercase text-[9px]">Company PAN:</span>
                                    <span className="font-black text-slate-900">AAFCD9999A</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-slate-500 font-bold uppercase text-[9px]">Company GSTIN:</span>
                                    <span className="font-black text-slate-900 text-base">24AAFCD9999A1ZP</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-2 text-center text-[10px] font-black uppercase tracking-widest border-x border-slate-900 border-b text-slate-400">
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {regFeeItem && (
                                <tr>
                                    ...
                                    <td className="p-3 border-r border-slate-900">
                                        <div className="font-black text-slate-800">{regFeeItem.description}</div>
                                        <div className="text-[9px] text-slate-400 font-bold uppercase">One-time facility setup & registration</div>
                                    </td>
                            ...
                            {fileItems.length > 0 && (
                                <tr>
                                    ...
                                    <td className="p-3 border-r border-slate-900">
                                        <div className="font-black text-slate-800 uppercase">Record Processing Fees</div>
                                        <div className="text-[10px] text-slate-500 font-bold">Digitization & Cloud Archiving of {fileItems.length} Patient MRDs</div>
                                    </td>
                                    <td className="p-3 border-r border-slate-900 text-center font-bold">998311</td>
                            ...
                            {nonFileItems.map((item: any, idx: number) => {
                                const srNo = (regFeeItem ? 1 : 0) + (fileItems.length > 0 ? 1 : 0) + idx + 1;
                                return (
                                    <tr key={idx}>
                                        <td className="p-3 border-r border-slate-900 text-center font-black text-slate-400">{srNo.toString().padStart(2, '0')}</td>
                                        <td className="p-3 border-r border-slate-900">
                                            <div className="font-black text-slate-800 uppercase">{item.description}</div>
                                        </td>
                            ...
                            <tr>
                                <td colSpan={4} className="p-5 bg-slate-50 border-y border-slate-900">
                                    ...
                                    <div className="font-black text-slate-900 text-xs uppercase tracking-tight italic">
                                        {invoice.amount_in_words || 'NA'}
                                    </div>
                                </td>
                            </tr>
                        </tbody>
                    ...
                    <div className="grid grid-cols-3 mt-8 border border-slate-900 overflow-hidden rounded-sm print:break-inside-avoid">
                        <div className="col-span-2 p-8 border-r border-slate-900 bg-slate-50/30">
                            ...
                            <div className="grid grid-cols-2 gap-x-12 gap-y-5">
                                <div>
                                    <span className="text-slate-400 font-bold uppercase block text-[8px] tracking-widest mb-1">Bank Name</span>
                                    <span className="font-black text-slate-900 text-xs">HDFC Bank Ltd.</span>
                                </div>
                                <div>
                                    <span className="text-slate-400 font-bold uppercase block text-[8px] tracking-widest mb-1">IFS Code</span>
                                    <span className="font-black text-slate-900 text-xs tracking-wider">HDFC0001234</span>
                                </div>
                                <div>
                                    <span className="text-slate-400 font-bold uppercase block text-[8px] tracking-widest mb-1">Account Number</span>
                                    <span className="font-black text-slate-900 text-sm tracking-[0.2em]">50200012345678</span>
                                </div>
                                <div>
                                    <span className="text-slate-400 font-bold uppercase block text-[8px] tracking-widest mb-1">Account Beneficiary</span>
                                    <span className="font-black text-slate-900 text-xs uppercase">Digifort Labs Pvt. Ltd.</span>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-8 print:mt-0 print:break-before-page">
                    <div className="bg-slate-900 text-white px-4 py-1.5 font-black text-[9px] uppercase tracking-[0.1em] mb-2 flex justify-between items-center">
                        <span className="flex items-center gap-2">
                            ...
                            <span className="text-indigo-400 opacity-60 font-bold text-[7px] tracking-normal px-2 border-l border-white/20">Detailed Manifest</span>
                        </span>
                        <div className="text-right flex items-center gap-2">
                            <span className="text-[7px] font-bold uppercase opacity-50">Total Records:</span>
                            <span className="text-sm font-black">{fileItems.length}</span>
                        </div>
                    </div>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {fileItems.map((item: any, idx: number) => (
                                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="p-1 border-r border-slate-200 text-center font-bold text-slate-300">{(idx + 1).toString().padStart(2, '0')}</td>
                            ...
                            <tr className="bg-slate-900 text-white font-black uppercase text-[8.5px]">
                                <td colSpan={3} className="p-1.5 border-r border-white/20 text-right tracking-widest">Manifest Total</td>
