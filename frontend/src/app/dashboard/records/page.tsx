...
import { Search, Filter, Plus, FileText, ChevronRight, Upload, User, ArrowRight, Pencil, Trash2 } from 'lucide-react';
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="bg-white border border-indigo-200 text-indigo-700 px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-50 transition shadow-sm"
                        >
                            <Upload size={20} /> <span className="hidden md:inline">Digitize Records</span>
                        </button>
                        <button
                            onClick={() => setShowCreateModal(true)}
                            className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/30"
                        >
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-100">
                        <tr>
                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-wider">Patient Name</th>
                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-wider">MRD / UHID</th>
                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-wider">Files</th>
                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-wider">Status</th>
                            <th className="p-6 text-xs font-black text-slate-400 uppercase tracking-wider text-right">Action</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {loading ? (
                            <tr><td colSpan={5} className="p-12 text-center">Loading Records...</td></tr>
                        ) : filteredPatients.length === 0 ? (
                            <tr><td colSpan={5} className="p-12 text-center text-slate-400 italic">No patients found. Create one to get started.</td></tr>
                        ) : (
