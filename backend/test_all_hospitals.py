...
from app.models import Patient, OPDVisit, IPDAdmission, DentalTreatment, PatientInvoice, Hospital
...
def test():
    ...
    try:
        ...
        for h in hospitals:
            ...
            print(f"\n--- Testing hospital_id: {hospital_id} ({h.legal_name}) ---")
            try:
                ...
                # Let's inspect dental query details
                # Join is done on DentalTreatment.patient_id == Patient.record_id
                ...
                print(f"ERROR FOR HOSPITAL {hospital_id}:")
                import traceback
                traceback.print_exc()
