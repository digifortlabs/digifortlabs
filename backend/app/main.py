...
async def verify_csrf(request: Request, csrf_protect: CsrfProtect = Depends()):
    ...
    """
    if request.url.path in ["/health", "/docs", "/redoc", "/openapi.json", "/auth/csrf-token", "/auth/token", "/optimization/trigger", "/optimization/ocr/result"] or request.url.path.startswith("/auth/"):
        return
