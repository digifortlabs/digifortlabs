...
def test_full_pipeline(file_path: str):
    # 1. Trigger
    print(f"🚀 Triggering optimization for {file_path}...")
    ...
    print(f"✅ Task Triggered. ID: {task_id}")
    ...
    while True:
        ...
        if status == "PROGRESS":
            ...
            print(f"   - {status}: {meta.get('step')} ({meta.get('percent')}%)")
        else:
            print(f"   - {status}")
    ...
    if status == "SUCCESS":
        ...
        print(f"📂 Downloading optimized file...")
        dl_url = f"{BASE_URL}/optimization/download/{job_id}/test_input.pdf"
        dl_resp = requests.get(dl_url)
        
        if dl_resp.status_code == 200:
            print(f"✅ Download Successful. Size: {len(dl_resp.content)} bytes")
        else:
            print(f"❌ Download Failed: {dl_resp.status_code} - {dl_resp.text}")
    else:
