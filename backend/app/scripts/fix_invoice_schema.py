...
def run_migration():
    with engine.connect() as conn:
        logger.info(f"Checking for missing columns in 'invoices' table on {DATABASE_URL.split('@')[-1]}...")
        ...
        try:
            ...
            logger.info(f"Could not retrieve table info using information_schema: {e}")
            existing_columns = []
        ...
        for col_name, col_type in columns_to_add:
            ...
            try:
                ...
                if "already exists" in str(e).lower():
                    logger.info(f"Column '{col_name}' already exists (caught exception).")
                else:
