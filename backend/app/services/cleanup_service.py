...
class CleanupService:
    ...
    def run_retention_policy(db: Session):
        ...
        """
        logger.info(f"[CLEAN] [Retention Cleanup] Started at {datetime.datetime.now()}")
        ...
        for hospital in hospitals:
            ...
            logger.info(f"[HOSPITAL] Hospital {hospital.legal_name}: Found {len(patients_to_cleanup)} patients past retention ({retention_years} yrs)")
            ...
            for patient in patients_to_cleanup:
                ...
                for f in patient.files:
                    try:
                        ...
                        logger.info(f"[WARN] Failed to delete file {f.file_id} during cleanup: {e}")
        ...
        if total_patients_deleted > 0:
            db.commit()
            logger.info(f"[OK] Cleanup Complete: Removed {total_patients_deleted} patients and {total_files_deleted} files.")
        else:
            logger.info("[NEW] Cleanup Finished: Nothing to remove.")
    ...
    def cleanup_temp_jobs(max_age_hours: int = 24, temp_dir_path: str = "backend/data/temp"):
        ...
        logger.info(f"[CLEAN] [Job Cleanup] Scanning {temp_dir} for expired jobs and lingering unencrypted files...")
        now = time.time()
            ...
            if item.is_dir():
                ...
                if (now - item.stat().st_mtime) > max_age_sec:
                    try:
                        ...
                        logger.info(f"[WARN] Failed to remove expired job dir {item}: {e}")
                        ...
                        if subitem.is_file() and subitem.suffix.lower() == ".pdf":
                            if (now - subitem.stat().st_mtime) > unencrypted_age_sec:
                                try:
                                    ...
                                    logger.info(f"[DELETE] Proactive Scrub: Lingering unencrypted file {subitem.name} removed.")
                                except Exception as e:
                                    logger.info(f"[WARN] Failed to proactively scrub {subitem}: {e}")
                except Exception as e:
                    logger.info(f"[WARN] Error scanning subdirectory {item} for intermediate PDFs: {e}")
        
        if removed_count > 0:
            logger.info(f"[OK] Job Cleanup Complete: Removed {removed_count} expired job directories.")
        if unencrypted_scrubbed_count > 0:
            logger.info(f"[CLEAN] Lingering Unencrypted Scrub Complete: Removed {unencrypted_scrubbed_count} raw/intermediate PDF files.")
