...
> `D:/Website/DIGIFORTLABS/.gemini/get-shit-done/USER-PROFILE.md` exists). Skip the Read
...
```bash
PROFILE_CONTENT=$(cat "D:/Website/DIGIFORTLABS/.gemini/get-shit-done/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @D:/Website/DIGIFORTLABS/.gemini/agents/gsd-advisor-researcher.md for your role and instructions.
