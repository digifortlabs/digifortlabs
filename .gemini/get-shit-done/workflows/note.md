...
- **Global scope**: `D:/Website/DIGIFORTLABS/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Website/DIGIFORTLABS/.gemini/notes/`)
...
2. Glob `D:/Website/DIGIFORTLABS/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (D:/Website/DIGIFORTLABS/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Website/DIGIFORTLABS/.gemini/notes/` — works in any directory
