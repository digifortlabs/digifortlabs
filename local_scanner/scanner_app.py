...
"""
    ...
    def _upload_worker(self):
        try:
            ...
            import re
            # Sanitize MRD Number for use as filename
            safe_mrd = re.sub(r'[^a-zA-Z0-9_\-]', '_', str(self.mrd_number))
            # Fallback if MRD is empty, N/A, or invalid
            if not safe_mrd or safe_mrd.upper() == 'N_A':
                 safe_mrd = "scan"
            
            base_name = safe_mrd
            # --- FILENAME LOGIC END ---

            # LOCAL SAVE (User Request: output/{MRD}.pdf)
            try:
                ...
                # If file exists, append counter: MRD.pdf -> MRD_01.pdf
            ...
            buf.seek(0)
            # Use the same sanitized filename for upload
            upload_filename = f"{base_name}.pdf"
            response = requests.post(url, headers=headers, files={'file': (upload_filename, buf)}, timeout=300) # Increased timeout for large files
